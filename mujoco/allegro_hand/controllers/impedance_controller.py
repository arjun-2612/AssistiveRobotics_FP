...
Implements Section III.A from the paper:
"Object-Level Impedance Control for Dexterous In-Hand Manipulation"

The controller works in contact space, computing desired contact forces
that move the object toward a desired pose while maintaining grasp configuration.
"""
...
class ImpedanceController:
    ...
    4. Apply spring-damper: f_x = K_x Δc + D_x Δċ
    
    From Equations (10)-(14) in the paper.
    """
    
    def __init__(self, contact_stiffness: float = 100.0, contact_damping: float = 10.0, n_contacts: int = 4):
        """
        ...
        Args:
            ...
            n_contacts: Number of contact points (default: 4 for 4-finger)
        """
        self.K_x = contact_stiffness
        self.D_x = contact_damping
        self.n_contacts = n_contacts
        
        # Store initial contact configuration (set during initialization)
        self.c_0 = None  # Initial contact positions (3n vector)
        
    def set_initial_contacts(self, contact_positions: np.ndarray):
        """
        Store the initial grasp configuration.
        
        Args:
            contact_positions: Initial contact positions (3n vector: [c1_x, c1_y, c1_z, c2_x, ...]) 
                                                                    Shape: (3n,) where n = number of contacts
        """
    ...
    def compute_contact_forces(self, x_current: np.ndarray, x_desired: np.ndarray,contact_positions: np.ndarray,
        contact_velocities: np.ndarray, grasp_matrix: np.ndarray, W_matrix: np.ndarray) -> np.ndarray:
        """
        ...
        # f_x = K_x Δc + D_x Δċ
        f_x = self.K_x * delta_c + self.D_x * contact_velocities
        
        return f_x
    
    def _compute_grasp_maintenance_term(self, contact_positions: np.ndarray, grasp_matrix: np.ndarray) -> np.ndarray:
        """
        ...
        Args:
            contact_positions: Current contact positions (3n)
            grasp_matrix: G matrix (6 x 3n)
            
        Returns:
            delta_c_f: Grasp maintenance term (3n)
        """
        ...
        I = np.eye(n_dim)
        nullspace_projector = I - G_T @ G_T_pinv
    ...
    def compute_W_matrix(euler_angles: np.ndarray) -> np.ndarray:
        ...
        # Euler angle Jacobian (maps body angular velocity to Euler angle rates)
        # For ZYX Euler angles (intrinsic rotations)
        c_r = np.cos(roll)
        s_r = np.sin(roll)
        c_p = np.cos(pitch)
        s_p = np.sin(pitch)

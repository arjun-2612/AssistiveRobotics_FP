...
with viewer.launch_passive(model, data) as v:
    ...
    while v.is_running():
        ...
        if len(contacts) >= 2:
            ...
            # τ = J^T f
            tau_qp = J.T @ f_contact

            # Position control to maintain grasp (critical!)
            K_pos = 20.0  # Position stiffness
            K_damp = 2.0   # Damping
            q_error = q_target - data.qpos[:16]
            qd = data.qvel[:16]
            tau_hold = K_pos * q_error - K_damp * qd

            # Combine: QP forces + position holding
            tau_total = tau_qp
            ...
            # tau_total = tau_contact + tau_position            
            ...
            if step % 100 == 0:
                ...
                print(f"  QP torque: {np.abs(tau_qp).max():.2f} Nm")
                print(f"  Hold torque: {np.abs(tau_hold).max():.2f} Nm")
                print(f"  Total torque: {np.abs(data.ctrl[:16]).max():.2f} Nm")
                
                if abs(obj_vel) < 0.001 and abs(height_error) < 0.002:
                    print(f"  ✓ Object stable!")
        else:
            # No contacts - apply position holding only
            q_error = q_target - data.qpos[:16]
            qd = data.qvel[:16]
            tau_hold = 20.0 * q_error - 2.0 * qd
            data.ctrl[:16] = np.clip(tau_hold, -5, 5)

...
def test_4_contacts():
    """Test with 4 contacts (more common realistic scenario)."""
    print("\n" + "="*60)
    print("TEST 8: Four-Contact Grasp")
    print("="*60)
    
    grasp = GraspMatrix(n_contacts=4)
    object_pos = np.array([0.0, 0.06, 0.10])
    
    # 4 contacts in tetrahedral formation
    contact_positions = np.array([
        [0.03, 0.05, 0.10],    # Index (front-right)
        [0.01, 0.07, 0.11],    # Middle (front-left, higher)
        [-0.01, 0.07, 0.10],   # Ring (back-left)
        [-0.04, 0.05, 0.09]    # Thumb (back-right, opposing)
    ])
    
    print(f"Object position: {object_pos}")
    print(f"Contact positions:")
    finger_names = ['Index', 'Middle', 'Ring', 'Thumb']
    for i, (pos, name) in enumerate(zip(contact_positions, finger_names)):
        offset = pos - object_pos
        print(f"  {name:6s} (contact {i}): {pos}  [offset: {offset}]")
    
    G = grasp.compute_grasp_matrix(contact_positions, object_pos)
    
    print(f"\nGrasp Matrix shape: {G.shape}")
    print(f"Expected shape: (6, 12)  [6 wrench DOF × 12 force components]")

    if G.shape == (6, 12):
        print("✅ PASS: Correct dimensions for 4-finger grasp!")
    else:
        print(f"❌ FAIL: Expected (6, 12), got {G.shape}")
    ...
    if has_force_closure:
        print("✅ PASS: 4-contact grasp has force closure!")
        print("   (Can control all 6 DOF of object motion)")
    else:
        print("⚠️  WARNING: Grasp lacks force closure")
        print("   (Contact configuration may be degenerate)")

    # Check nullspace dimension
    nullspace_dim = 12 - rank  # 12 force components - 6 constrained DOF
    print(f"\nNullspace dimension: {nullspace_dim}")
    print(f"   (Number of independent internal force patterns)")
    
    if nullspace_dim >= 3:
        print("✅ Good redundancy: Can apply internal forces in multiple ways")
    elif nullspace_dim > 0:
        print("⚠️  Limited redundancy")
    else:
        print("⚠️  No redundancy: Grasp is minimally constrained")
